...
class Preferred(Preferred):
    ...
    def __init__(self, parent=None):
        ...
        self.loading = LoadingWindow("正在加载", "[DefaultText]", self.parent_)
    ...
    def changeItem(self, item, column):
        ...
        items = [self.currentMoudle[1], self.currentChapter[1], self.currentPoint[1]]
        text = ""
        for item in items:
    ...
    def showCateGory(self):
        ...
        if subject_cate is None:
            ...
            logger.error(f"showCateGory error: subject '{currentSubject}' not found.")
        ...
        if state_cate is None:
            logger.error(
                f"showCateGory error: subject '{currentSubject}' state '{currentState}' not found."
            )

            InfoBar.error(
                "加载错误",
                f"未找到学科 {currentSubject} 的阶段 {currentState}",
                parent=self,
            )
        ...
        logger.info(f"Getting {currentState}: {currentSubject} categories.")
        ...
        for moudle in state_cate["items"]:
                ...
                def finished(message):
                    if message[0]:
                        for point in message[1]["data"]:
                            ...
                            if config.get("cache.cache_categories", False):
                                pass
                    ...
                    if all:
                        logger.info(
                            f"Get {len(moudle["items"])} points for chapter all finished."
                        )
                        self.loading.close()
            ...
            logger.info(f"Getting {len(moudle["items"])}points for chapter")
    ...
    def search(self):
        ...
        self.search_worker = GetPapersListWorker(
            self.page, subject, grade, logger=logger
        ).setType(store_type)
        self.search_worker.setType(store_type).setYear(year).setModule(
            *self.currentMoudle
        )
        self.search_worker.setChapter(*self.currentChapter).setPoint(*self.currentPoint)
        self.search_worker.setAssembly(assembly_grade, assembly_type).setCatid(
            self.currentMoudle[0], self.currentChapter[0], self.currentPoint[0]
        )
